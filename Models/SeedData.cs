    ...
    public static class SeedData
    {
        public async static Task Initialize(IServiceProvider serviceProvider)
        {
            ...
            if(!context.Routes.Any())
            {
                ...
                route.ScheduledStops.Enqueue(scheduledStop1);
                route.ScheduledStops.Enqueue(scheduledStop2);
                route.ScheduledStops.Enqueue(scheduledStop3);
                route.ScheduledStops.Enqueue(scheduledStop4);
                route.ScheduledStops.Enqueue(scheduledStop5);
                context.Routes.Add(route);
                Route route2 = new Route();
                route2.Name = "Pembina";
                route2.BicycleAccessible = true;
                route2.RampAccessible = true;
                route2.ScheduledStops.Enqueue(scheduledStop3);
                route2.ScheduledStops.Enqueue(scheduledStop4);
                route2.ScheduledStops.Enqueue(scheduledStop1);
                route2.ScheduledStops.Enqueue(scheduledStop2);
                route2.ScheduledStops.Enqueue(scheduledStop5);
                context.Routes.Add(route);
